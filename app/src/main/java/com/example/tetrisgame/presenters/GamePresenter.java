...
public class GamePresenter {
    ...
    public void init() {
        mGameModel.init();
        mGameView.init(mGameModel.getGameSize(), mGameModel.getHighScore());
    ...
    public void init(Context context) {
        mGameModel.init(context); // Initialize with context
        mGameView.init(mGameModel.getGameSize(), mGameModel.getHighScore()); // Pass high score to GameView
        mGameModel.setGameOverListener(() -> {
            setStatus(GameStatus.OVER);
            mGameModel.updateHighScore(mGameView.getScore()); // Update high score on game over
        });
        mGameModel.setScoreUpdatedListener(mGameView::setScore);
        setStatus(GameStatus.START);
    }
